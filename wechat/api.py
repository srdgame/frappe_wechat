...
def send_device_alarm(app, user_list, alarm):
	...
	url = WeChatOAuth(app_id, secret, domain + "/issue_list/" + app + "?name=" + alarm["name"]).authorize_url
...
def send_repair_issue(app, user_list, issue):
	...
	url = WeChatOAuth(app_id, secret, domain + "/issue_list/" + app + "?name=" + issue["name"]).authorize_url
	for user in user_list:
		user_id = frappe.get_value("Wechat Binding", {"app": app, "user": user}, "openid")
		if not user_id:
			continue
